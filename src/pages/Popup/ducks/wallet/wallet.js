...
export default function reduceWallet(state = {}, action) {
  const walletState = {
    isInitialized: false,
    isUnlocked: false,
  ...
  switch (action.type) {
    case actionConstants.UPDATE_WALLET_STATE:
      return { ...walletState, ...action.value };
    ...
    case actionConstants.SET_ACCOUNT_LABEL: {
      ...
      const id = {};
      id[account] = { ...walletState.identities[account], name };
      const identities = { ...walletState.identities, ...id };
      return Object.assign(walletState, { identities });
    }
    ...
    case actionConstants.TOGGLE_ACCOUNT_MENU:
      return {
        ...walletState,
        isAccountMenuOpen: !walletState.isAccountMenuOpen,
      };

    case actionConstants.UPDATE_TRANSACTION_PARAMS: {
      const { id: txId, value } = action;
      let { currentNetworkTxList } = walletState;
...
export const getCurrentLocale = (state) => state.WALLET.currentLocale;

export const getAlertEnabledness = (state) => state.WALLET.alertEnabledness;
...
export function getBlockGasLimit(state) {
  return state.WALLET.currentBlockGasLimit;
}

export function getConversionRate(state) {
  return state.WALLET.conversionRate;
}

export function getNativeCurrency(state) {
  return state.WALLET.nativeCurrency;
}

export function getSendHexDataFeatureFlagState(state) {
  return state.WALLET.featureFlags.sendHexData;
}
...
export function getUnapprovedTxs(state) {
  return state.WALLET.unapprovedTxs;
}
...
export function isNotEIP1559Network(state) {
  return state.WALLET.networkDetails?.EIPS[1559] === false;
}
...
export function isEIP1559Network(state) {
  return state.WALLET.networkDetails?.EIPS[1559] === true;
}

export function getGasEstimateType(state) {
  return state.WALLET.gasEstimateType;
}

export function getGasFeeEstimates(state) {
  return state.WALLET.gasFeeEstimates;
}

export function getEstimatedGasFeeTimeBounds(state) {
  return state.WALLET.estimatedGasFeeTimeBounds;
}
...
export function getCompletedOnboarding(state) {
  return state.WALLET.completedOnboarding;
}
export function getIsInitialized(state) {
  return state.WALLET.isInitialized;
}

export function getIsUnlocked(state) {
  return state.WALLET.isUnlocked;
}

export function getSeedPhraseBackedUp(state) {
  return state.WALLET.seedPhraseBackedUp;
}
...
export function findKeyringForAddress(state, address) {
  const keyring = state.WALLET.keyrings.find((kr) => {
...
export function getLedgerTransportType(state) {
  return state.WALLET.ledgerTransportType;
}
...
/**
 * Given the redux state object, returns a boolean indicating whether the user has any Ledger accounts added to WALLET (i.e. Ledger keyrings
 * in state)
 *
 * @param {Object} state - the redux state object
...
export function doesUserHaveALedgerAccount(state) {
  return state.WALLET.keyrings.some((kr) => {
