...
/**
 * An ObservableStore that can compose a flat
 * structure of child stores base on configuration
 */
export default class ComposableObservableStore extends ObservableStore {
   ...
   *  messenger, used for subcribing to events from BaseControllerV2-based
   *  controllers.
  ...
  /**
   * Composes a new internal store subcription structure
   * @param {Record<string, Object>} config - Describe which stores are beging composed
   * The key is the name of the store, and the value is either an
      ...
      if (store.subscribe) {
        ...
        this.controllerMessenger.subscribe(
          ...
          (state) => {
            ...
            this.updateState({ [key]: this.updateState });
          },
        );
      }
    }
   ...
   * returning an object keyed id by child store class name
